...
class FocusClient:
    def __init__(self, settings: Settings):
        ...
        limits = httpx.Limits(max_connections=settings.focus_max_connections)
        common = dict(cookies=self.cookies, follow_redirects=True, verify=settings.focus_verify_tls, timeout=settings.focus_timeout_seconds, limits=limits)
        self.http = httpx.AsyncClient(base_url=settings.focus_base_url, headers=headers, **common)
        self.auth_http = httpx.AsyncClient(base_url=settings.auth_base_url, **common)
    ...
    async def authenticate(self, force: bool = False):
        if not self.s.configured: raise FocusConfigurationError('FOCUS_BASE_URL, FOCUS_AUTH_BASE_URL, FOCUS_USERNAME and FOCUS_PASSWORD are required')
        ...
        async with self._auth_lock:
            ...
            try:
                if self.s.focus_auth_mode.lower() == 'json': r = await self.auth_http.post(self.s.focus_login_path, json=payload)
                else: r = await self.auth_http.post(self.s.focus_login_path, data=payload)
            ...
            if self.s.focus_session_cookie and self.s.focus_session_cookie not in self.cookies:
    ...
    async def request(self, method: str, path: str, **kwargs) -> Any:
        ...
        for attempt in range(2):
            ...
            try: return r.json()
            except ValueError: return {'ok': True, 'status_code': r.status_code, 'url': str(r.url), 'content_type': r.headers.get('content-type',''), 'text': r.text[:2000]}
        raise FocusRequestError('Request failed after reauthentication')

    async def health(self): return {'configured': self.s.configured, 'base_url': self.s.focus_base_url, 'auth_base_url': self.s.auth_base_url, 'login_path': self.s.focus_login_path}
